...
if [[ $HOST = "jjuhaszQJHD2.vmware.com" ]]; then 
    PROMPT='${NEWLINE}%{%F{magenta}%}thirteen %{%F{yellow}%}[%1~] ${git_dir_status}%B%(?.%F{green}.%F{red})%#%f %{$reset_color%}'
fi
...
if [[ $HOST = "jump" || $HOST = "arch" ]]; then
    ...
    alias k6='cp ~/.kube/config.clusterbk ~/.kube/confi'
